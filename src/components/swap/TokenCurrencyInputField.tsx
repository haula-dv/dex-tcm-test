...
const TokenCurrencyInputField = ({
  ...
  const { data: inputMarkPrice } = useMarkPrice(
    `PERP_${sellTokenActive?.token}_USDC`
  );
  const { data: outputMarkPrice } = useMarkPrice(
    `PERP_${buyTokenActive?.token}_USDC`
  );
  ...
  return (
    <>
      ...
      <ContentCurrencyField
        spacing={"4px"}
        isActived={sellTokenActive?.isInputting}
      >
        <Stack
          direction={"row"}
