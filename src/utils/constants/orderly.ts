...
// 	const networdId = localStorage.getItem('networdId') ?? 'testnet';
// 	if (networdId == 'testnet') {
// 		return true;
// 	} else {
// 		return false;
// 	}
...
export function getVerifyingAddress(chainId: string): string {
  return match(isTestnet(chainId as any))
...
export function getBaseUrl(networkId: string): string {
  return match(networkId == "testnet")
    .with(false, () => "https://api-evm.orderly.org/v1")
    .with(true, () => "https://testnet-api-evm.orderly.org/v1")
    .exhaustive();
}
